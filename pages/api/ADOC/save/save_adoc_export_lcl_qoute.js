...
export default async function handler(req, res) {
  ...
  const quoteData = {
    ...
    D_CUC: parseDecimal(destinationData[0]?.[20]),
    D_CCF: parseDecimal(destinationData[1]?.[20]),
    D_DOC: parseDecimal(destinationData[2]?.[20]),
    D_AAI: parseDecimal(destinationData[3]?.[20]),
  ...
  try {
    ...
    await prisma.$executeRaw`EXEC [dbo].[SaveADOCExportLCLQuote] @jsonval = ${JSON.stringify(
      quoteData
    )}`;
