...
export default async function handler(req, res) {
    ...
    if (req.method === "POST") {
        try {
            ...
            await prisma.$executeRaw`EXEC [dbo].[Save_Mumbai_Quote] ${vehicleType}, ${currency}, ${parseFloat(rate)}, ${supplierCode}, ${createdBy}, ${parseInt(quoteYear)}`;
            
            return res.status(200).json({ message: "Quote saved successfully" });

        } catch (error) {
            console.error('Error saving Mumbai quote:', error);
