...
export const exportPunePdf = (tableData, quoteYear) => {
    const doc = new jsPDF({
    ...
    const signatureLine = "_____________";

    doc.text('Comparitive Statement of quotations', margin, 15);
    doc.text(`Date: ${currentDate}`, pageWidth - margin, 15, { align: 'right' });
    
    doc.text(`RFQ rates for GTI to Pune shipment for ${quoteYear} ( January to December )`, margin, 22);
    ...
    const mainTableHead = [
        [
            { content: `January to December '${quoteYear} Quote for GTI to Garrett Pune`, colSpan: 3, styles: { halign: 'center', fontStyle: 'bold', fontSize: 16, cellPadding: 4 } }
        ],
        [
    ...
    doc.save(`GTI_to_Pune_Quote_${quoteYear}.pdf`);
};