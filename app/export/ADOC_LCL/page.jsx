...
const QuotationTable = () => {
  ...
  const fetchQuotationData = async (locationCode) => {
    try {
      const response = await fetch("/api/ADOC/get/get_adoc_export_lcl_quote", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const saveQuote = async (containerSize) => {
    ...
    try {
      ...
      const response = await fetch("/api/ADOC/save/save_adoc_export_lcl_qoute", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
          ...
          <table className="table-auto border-[var(--primary)] text-center w-full min-w-[800px] text-xs">
            <thead className="bg-[var(--bgBody3)] text-[var(--buttonHover)] border border-[var(--bgBody)]">
              <tr> 
                ...
                <th rowSpan="2" className="py-1 px-2 border border-[var(--bgBody)] text-orange-500 ">Sea Freight Export ADOC LCL</th>
