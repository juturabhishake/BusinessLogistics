...
const QuotationTable = () => {
    ...
    const fetchCurrency = async () => {
      try {
        ...
        if (data.result && data.result.length > 0) {
          ...
          console.log("OLD Currency fetched:", data.result[0]);
        }
      } catch (error) {
  ...
  const downloadPDF = () => {
    ...
    // tableBody.push([{ content: "Uploaded PDF", colSpan: 2, styles: { fontStyle: "bold" } }, { content: uploadedPdfPath, colSpan: 8 }]);
    ...
    doc.setFontSize(8);
    doc.text(`Air Export rates`, 5, 14, { align: "left" });
      ...
      <div className="card-header bg-[var(--bgBody)] text-white rounded-t-lg py-2 px-3">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center">
            <div className="flex flex-col">
              ...
              <p className="text-xs text-gray-100">Air Export rates</p>
          ...
          <table className="table-auto border-[var(--primary)] text-center w-full min-w-[800px] text-xs">
            <thead className="bg-[var(--bgBody3)] text-[var(--buttonHover)] border border-[var(--bgBody)]">
              <tr> 
                ...
                <th rowSpan="2" className="py-1 px-2 border border-[var(--bgBody)] text-orange-500 ">Air Freight Export</th>
