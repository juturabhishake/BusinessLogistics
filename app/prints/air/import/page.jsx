...
const QuotationTable = () => {
  ...
  const fetchQuotationData = async (locationCode) => {
    try {
      ...
      const response = await fetch("/api/userPrints/airImport", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const downloadPDF = () => {
    ...
    const tableHeaders = [
      [
        ...
        { content: `Quote for ${locationName || "{select location}"} to GTI`, colSpan: 2, styles: { halign: "center" } },
    ...
    const maxDeliveryAddressLength = 300;
    ...
    tableBody.push([
      ...
      { content: cleanedDeliveryAddress, colSpan: 8, styles: { fontSize: 7 } }
    ]);
    ...
    doc.setFontSize(8);
    doc.text(`Air Import rates`, 5, 14, { align: "left" });
    const loc = locationName.split('|')[0].trim();
    doc.text(`Quote for  ${loc} to GTI Air shipment`, 5, 18, { align: "left" });
    doc.setFontSize(7);
      ...
      <div className="card-header bg-[var(--bgBody)] text-white rounded-t-lg py-2 px-3">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center">
            <div className="flex flex-col">
              ...
              <p className="text-xs text-gray-100">Air Import rates</p>
          ...
          <table className="table-auto border-[var(--primary)] text-center w-full min-w-[800px] text-xs">
            <thead className="bg-[var(--bgBody3)] text-[var(--buttonHover)] border border-[var(--bgBody)]">
              <tr> 
                ...
                <th rowSpan="2" className="py-1 px-2 border border-[var(--bgBody)] text-orange-500 ">Air Freight Import </th>
                <th rowSpan="2" className="py-1 px-2 border border-[var(--bgBody)]">Currency in</th>
                <th colSpan="2" className="py-1 px-2 border border-[var(--bgBody)]">Quote for {locationName || "{select location}"} to GTI  Air shipment</th>
