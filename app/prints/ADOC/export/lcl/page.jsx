...
const QuotationTable = () => {
  ...
  const fetchQuotationData = async (locationCode) => {
    try {
      ...
      const response = await fetch("/api/userPrints/ADOC_Export_LCL", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const downloadPDF = () => {
    ...
    doc.setFontSize(8);
    doc.text(`Adhoc Export rates for Adhoc LCL`, 5, 14, { align: "left" });
      ...
      <div className="card-header bg-[var(--bgBody)] text-white rounded-t-lg py-2 px-3">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center">
            <div className="flex flex-col">
              ...
              <p className="text-xs text-gray-100">"RFQ Import rates for Adhoc LCL"</p>
          ...
          <table className="table-auto border-[var(--primary)] text-center w-full min-w-[800px] text-xs">
            <thead className="bg-[var(--bgBody3)] text-[var(--buttonHover)] border border-[var(--bgBody)]">
              <tr> 
                ...
                <th rowSpan="2" className="py-1 px-2 border border-[var(--bgBody)] text-orange-500 ">Sea Freight RFQ - LCL</th>
