...
const AdhocLocations = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("/api/ADOC/get_adhoc_locations");
        if (!response.ok) {
  ...
  const handleSave = async () => {
    if(formData.Location === ""  || formData.Actual_Location === ""  || formData.Country === "" || formData.Currency === "" || formData.Loc_Address === "") {
      alert("required all inputs!!!");
      return
    }
    setSaveState("saving");
    try {
      const response = await fetch("/api/ADOC/update_adhoc_location", {
        method: "POST",
        headers: {
  ...
  const handleAddNewLocation = async () => {
    ...
    setSaveState("saving");
    try {
      const response = await fetch("/api/ADOC/add_newadhoc_location", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
                  ...
                  {paginatedData.length > 0 ? (
                    paginatedData.map((item) => (
                      <tr key={item.Location_Id} className="border hover:bg-muted">
                        ...
                        <td className="px-4 py-2 border">{item.Actual_Location}</td>
                        <td className="px-4 py-2 border">{item.Country}</td>
                         <td className="px-4 py-2 border">{item.Loc_Address}</td>
          ...
          >
             <div className="bg-gray-200 dark:bg-gray-700 p-4 rounded-t-lg">
              ...
              <div>
               ...
               <div>
                <label className="block text-sm font-semibold">Actual Location</label>
                <input
                  type="text"
                  className="w-full p-2 border rounded"
              ...
              <div className="col-span-2">
                    <label className="block text-sm font-semibold">Delivery Address</label>
                     <textarea
                        ...
                        onChange={(e) => setFormData({ ...formData, Loc_Address: e.target.value })}
                      />
                  </div>
              </div>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {[                   
                    { label: "Location", key: "Location" },
                     { label: "Actual_Location", key: "Actual_Location" },                     
