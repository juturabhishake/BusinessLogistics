...
export const exportMumbaiAdminPdf = (tableData, vendorColumns, quoteYear) => {
    const doc = new jsPDF({
    ...
    doc.text(`RFQ rates for Mumbai to GTI shipment (01.01.${quoteYear} - 31.12.${quoteYear})`, margin, 22);
    ...
    const mainTableHead = [
        [
            { content: `January to December '${quoteYear} Quote for Mumbai to GTI`, colSpan: vendorColumns.length + 2, styles: { fontSize: 16 } }
        ],
        [
 ...
 doc.text("GREENTECH INDUSTRIES Business @2023.04.03 by Muni Kranth.", doc.internal.pageSize.width / 2, doc.internal.pageSize.height - 8, { align: "center" });
    doc.save(`Mumbai_to_GTI_Admin_Quote_${quoteYear}.pdf`);
};