...
const QuotationTable = () => {
  ...
  const fetchQuotationData = async (locationCode, month, year, CBM) => {
    try {
        const response = await fetch("/api/prints/ImportLCL", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
        ...
        if (CBM >= 1 && CBM <= 6) {
            ...
            ["O_CC", "O_CCF", "O_DOC", "O_CFS", "O_LU", "O_Del"].forEach((attribute, index) => {
                for (let i = 0; i < 3; i++) {
            ...
            ["D_CCD", "D_LTS", "D_THC", "D_BLC", "D_LUS"].forEach((attribute, index) => {
                for (let i = 0; i < 3; i++) {
